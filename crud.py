...
def get_tours(skip: int = Query(default=0, ge=0), limit: int = Query(default=10, gt=0), search: str = '',
              any_user=None) -> list[SavedTour]:
    saved_tours = storage.get_tours(skip, limit, search)
    return saved_tours


@app.get('/api/get-tours/{tour_id}')
def get_tours(tour_id: str, any_user: Annotated[User, Depends(get_current_user)]) -> SavedTour:
